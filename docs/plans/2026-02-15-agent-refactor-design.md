# Agent refactor design: context factory, tool factories, builder, chat module
...
- **Chat module (new / extracted):** Single entry point for “one conversation action”: create turn, load history, call Agent streaming, persist assistant reply or error, refresh title. Depends on: Agent Builder, turn/message storage (existing DB + `ConversationService`), and optionally context estimation.
- **Agent Builder:** Only “build Agent from context + tools”. Input: system prompt (from context factory), tool list (built-in + MCP). Output: cached `AIAgent` (normal + thinking) and Invalidate. No knowledge of conversations, turns, or DB.
- **Supply layer:** Three stateless “factories”:
  ...
  - **MCP tool factory:** From `IMcpConfigService`, creates stdio/HTTP clients and `ListToolsAsync`, returns `(AITool[], IAsyncDisposable[] clients)`; builder or Chat owns disposal on Invalidate/Dispose.

Skills participate only in the context factory; MCP only in the tool list; built-in tools in their own factory; Agent is built by Builder; Chat is pure conversation orchestration.
...
**Agent Builder**  
- Interface/class: `IAgentBuilder` or `AgentBuilder`.  
...
- Interface: `IChatAgentService` or `IConversationAgentService`. Conversation-level API:  
  ...
  - `Task InvalidateAgentAsync()` (forward to builder)  
- Implementation depends on: `IAgentBuilder`, `AppDbContext` (or abstract message/turn storage), `ConversationService`, `ITokenizer`, `IConfiguration`.  
...
- `GetEstimatedContextUsageAsync` uses builder’s `GetCachedSystemPromptForTokenCount()` (or context factory cache) + `ChatMessageStoreAdapter` + tokenizer to compute ratio.
...
2. Chat module writes DB: create `ConversationTurn`, insert user `ChatMessage`; if first message, call title generation (one `RunAsync` via builder), then `SaveChangesAsync`, return `turnId`.  
3. UI calls `SendMessageStreamingAsync(...)`.  
4. Chat module: load history with `ChatMessageStoreAdapter`, convert to framework `ChatMessage` list and append current user message; call `IAgentBuilder.GetOrCreateAgentAsync(useThinking, ct)`; run `RunStreamingAsync(messages, ...)`, map content to `StreamUpdate` and yield.  
...
7. When MCP or Skills config changes: UI or settings calls `InvalidateAgentAsync()`; Chat forwards to `IAgentBuilder.InvalidateAsync()`; builder clears cache and Disposes MCP clients; next send rebuilds agent.

**Context usage**  
- UI calls `GetEstimatedContextUsageAsync(conversationId, ct)`.  
- Chat module uses builder’s (or context factory’s) cached system prompt, loads messages, serializes to request-shaped JSON, passes to `ITokenizer.CountTokens`, divides by `GetContextWindowTokens()`, returns 0–1.  
...
**Dependency direction:** UI → Chat module → builder and storage; builder uses context + built-in + MCP factories. Config UIs only call `InvalidateAgentAsync()`.
...
- Builder builds agent with built-in + successful MCP tools; one failing MCP does not fail the whole load.

**Missing API key**  
- Builder reads env in `GetOrCreateAgentAsync`; if empty, log Warning and still create client (e.g. empty string); first real request fails with auth error, Chat module catches and calls `CompleteTurnWithErrorAsync`.
...
**Dispose**  
- Chat module or root service that holds builder is disposed on shutdown; builder Disposes its MCP clients and clears agent references. Stateless factories need no IDisposable unless they hold non-managed resources.
