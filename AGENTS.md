...
- **Build** (from repo root): `dotnet build` or `dotnet build SmallEBot/SmallEBot.csproj`. Solution file: `SmallEBot.slnx`.
- **Run** (from repo root): `dotnet run --project SmallEBot`. API keys via env (e.g. `ANTHROPIC_API_KEY`, `DeepseekKey`); see README.
- **EF Core migrations** (from repo root): `dotnet ef migrations add <MigrationName> --project SmallEBot.Infrastructure --startup-project SmallEBot`. Migrations live under `SmallEBot.Infrastructure/Data/Migrations`; applied automatically on startup in `Program.cs`.
- **Lint**: Use the IDE/linter; no separate lint script.
- **Tests**: No test project in the repo.
...
- **Python**: Built-in **RunPython(code?, scriptPath?, workingDirectory?)** runs Python using **python.exe** in the run directory; pass inline code or a path to a `.py` file under the run directory; if both are provided, scriptPath is used.
- **Skills**: File-based skills under `.agents/sys.skills/` (system) and `.agents/skills/` (user). Each skill is a folder containing `SKILL.md` with YAML frontmatter (`name`, `description`). `SkillsConfigService` lists and parses metadata; **IAgentContextFactory** injects skill metadata into the system prompt. Built-in tools: **ReadSkill(skillName)** (loads SKILL.md by id from sys.skills or skills), **ReadFile(path)**, **WriteFile(path, content)**, **ListFiles(path?)** (list files and subdirectories under run directory), **ExecuteCommand**, **RunPython** (see Terminal and Python sections). Skills config UI in `Components/Skills/`. After adding/removing/importing skills, call `AgentCacheService.InvalidateAgentAsync()`.
