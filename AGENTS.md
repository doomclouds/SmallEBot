...
- **Theming**: Multi-theme via `data-theme` on `<html>`, `--seb-*` tokens in `wwwroot/app.css`, and `ThemeProviderHelper` + `ThemeConstants`; persistence in localStorage (`smallebot.theme`).
