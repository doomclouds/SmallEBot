...
- **Projects**: **SmallEBot.Core** (class library) — domain entities, `IConversationRepository`, shared models; no EF or Blazor. **SmallEBot.Infrastructure** (class library) — `SmallEBotDbContext`, EF migrations, `ConversationRepository`, BackfillTurns; references Core. **SmallEBot** (Blazor Server host) — UI, SignalR, DI; references Core and Infrastructure.
- **App**: Blazor Server (`.NET 10`) in `SmallEBot/`. Entry: `Program.cs`; UI: `Components/` (Razor + MudBlazor); app services in `Services/`. Data access is via **IConversationRepository** (implemented in Infrastructure); Host does not reference EF or DbContext directly.
- **Request flow**: Blazor UI (chat page, sidebar, dialogs) → SignalR → scoped services. **AgentService** is the chat module (create turn, stream, complete turn, context %, invalidate); it uses **IConversationRepository** and **IAgentBuilder**. AgentBuilder composes **IAgentContextFactory** (system prompt + skills block), **IBuiltInToolFactory** (GetCurrentTime, ReadFile, ReadSkill), and **IMcpToolFactory** (MCP tools + clients), then builds/caches the Anthropic agent. **ConversationService** delegates to **IConversationRepository** for all conversation/turn/message persistence. User identity is a single username (first-visit dialog), stored in `UserNameService` (session + `UserPreferencesService`); all conversation data is scoped by that username.
