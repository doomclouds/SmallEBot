...
- **Python**: Built-in **RunPython(code?, scriptPath?, workingDirectory?)** runs Python in an IronPython sandbox (no system Python required); pass inline code or a path to a `.py` file under the run directory; if both are provided, scriptPath is used.
- **Skills**: File-based skills under `.agents/sys.skills/` (system) and `.agents/skills/` (user). Each skill is a folder containing `SKILL.md` with YAML frontmatter (`name`, `description`). `SkillsConfigService` lists and parses metadata; **IAgentContextFactory** injects skill metadata into the system prompt. Built-in tools: **ReadSkill(skillName)** (loads SKILL.md by id from sys.skills or skills), **ReadFile(path)** and **WriteFile(path, content)** (allowed extensions under run directory: .md, .cs, .py, .txt, .json, .yml, .yaml). Skills config UI in `Components/Skills/`. After adding/removing/importing skills, call `AgentCacheService.InvalidateAgentAsync()`.
